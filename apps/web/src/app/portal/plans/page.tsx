...
type Product = { id: string; kind: "plan" | "traffic_pack"; status: string; name: string; description?: string | null; accent?: string | null; trafficReset: "monthly" | "never"; access: { profileName?: string | null; speedUpMbps: number; speedDownMbps: number; deviceLimit: number; nodePools: Array<{ id: string; name: string; region?: string | null; nodes: Array<{ id: string; label: string; serviceable: boolean }> }> }; offers: Offer[] };
...
type Branding = { purchaseMode: "balance" | "cdk"; buyButtonText: string; cdkButtonText: string; cdkButtonUrl: string };

const periodName = { monthly: "月付", quarterly: "季付", yearly: "年付", legacy: "固定期" } as const;
const defaultBranding: Branding = { purchaseMode: "balance", buyButtonText: "购买", cdkButtonText: "CDK 充值", cdkButtonUrl: "/portal/redeem" };
...
export default function PortalPlansPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [nextCatalog, nextWallet, nextBranding, nextOverview] = await Promise.all([
        apiRequest<Catalog>("/api/portal/catalog", { token }),
        apiRequest<Wallet>("/api/portal/wallet", { token }),
        apiRequest<Branding>("/api/portal/branding", { token }).catch(() => defaultBranding),
        apiRequest<PortalOverviewResponse>("/api/portal/subscription", { token }).catch(() => null),
      ]);
      setCatalog(nextCatalog); setWallet(nextWallet); setBranding(nextBranding); setCurrentPlanName(nextOverview?.plan.name ?? null);
      setSelected((current) => {
        const next = { ...current };
  ...
  const renderProducts = (products: Product[]) => (
    <section className="plan-grid">
      {products.map((product) => {
        ...
        const isCurrent = product.kind === "plan" && product.name === currentPlanName;
        return (
          <article className={`plan-card premium-plan-card${isCurrent ? " current" : ""}`} data-plan-accent={accent} key={product.id}>
            <ShaderAnimation color={planAccentColor(accent)} className="plan-card-shader" />
            <div className="plan-card-head">
              <div className="plan-card-copy"><h2 className="panel-title plan-card-title"><span aria-hidden="true" />{product.name}</h2><span className="panel-copy">{product.description || (product.kind === "plan" ? "标准会员套餐" : "独立流量权益")}</span></div>
              <div className="plan-price-block"><div className="price-line">{offer ? formatMoney(offer.priceCents) : "暂不可售"}</div><span className="fine-print">{offer ? offerPeriodName(offer) : "-"}</span></div>
            </div>
            <div className="panel-body">
              <div className="plan-spec-grid">
                <div className="plan-spec-item"><Icon name="bolt" /><span>上行<strong>{formatSpeedLimit(product.access.speedUpMbps)}</strong></span></div>
                <div className="plan-spec-item"><Icon name="monitoring" /><span>下行<strong>{formatSpeedLimit(product.access.speedDownMbps)}</strong></span></div>
                <div className="plan-spec-item"><Icon name="account_circle" /><span>设备<strong>{product.access.deviceLimit} 台</strong></span></div>
              </div>
              {offers.length > 1 ? <label className="field"><span className="fine-print">规格</span><CustomSelect value={offer?.id ?? ""} onChange={(value) => setSelected((current) => ({ ...current, [product.id]: value }))} options={offers.map((item) => ({ value: item.id, label: `${item.name} · ${formatMoney(item.priceCents)}` }))} /></label> : null}
              {offer ? <div className="plan-benefit-list">
                <div><span className="plan-benefit-icon"><Icon name="globe" /></span><span>可用节点</span><strong>{nodes.map((node) => node.label).join(" · ") || "未绑定"}</strong></div>
                <div><span className="plan-benefit-icon"><Icon name="network_node" /></span><span>{product.kind === "plan" ? "周期流量" : "一次性流量"}</span><strong>{formatTrafficLimit(offer.trafficBytes)}</strong></div>
              </div> : null}
              <div className="plan-card-footer"><button className="action-button" type="button" disabled={!offer} onClick={() => offer && openCheckout(product, offer)}>{isCurrent ? "续费套餐" : product.kind === "plan" ? branding.buyButtonText : "购买流量包"}</button><div className="plan-card-status">{isCurrent ? <span className="badge success">当前套餐</span> : product.kind === "traffic_pack" ? <span className="badge info">独立接入</span> : null}</div></div>
            </div>
          </article>
        );
      })}
  ...
  return (
    <ConsoleShell title="套餐与流量包" subtitle="统一权益商城" scope="Member" navItems={portalNav} requireRole="member" toolbarMeta={<span className="badge info">余额 {formatMoney(wallet.balanceCents)}</span>}>
      ...
      <div className="page-stack">
        ...
        {loading ? <section className="plan-grid">{Array.from({ length: 3 }, (_, index) => <div className="skeleton" style={{ height: 310, borderRadius: "var(--radius-md)" }} key={index} />)}</section> : renderProducts(groups.plans)}
        ...
        {!loading && !catalog.products.length ? <div className="empty-state"><div className="empty-state-title">当前没有可售商品</div></div> : null}
      </div>
      <Drawer open={Boolean(checkout)} onClose={() => setCheckout(null)} title={checkout ? `结算：${checkout.product.name}` : "结算"}
        ...
        {checkout ? <div className="form-grid">
          ...
          <div className="checkout-facts">
            <div><span>流量规则</span><strong>{checkout.product.kind === "plan" ? `每月重置 ${formatTrafficLimit(checkout.offer.trafficBytes)}` : `一次性总量 ${formatTrafficLimit(checkout.offer.trafficBytes)}`}</strong></div>
            ...
            <div><span>速率 / 设备</span><strong>{formatSpeedLimit(checkout.product.access.speedDownMbps)} · {checkout.product.access.deviceLimit} 台</strong></div>
          </div>
          ...
          <div className="toolbar-actions">
            {branding.cdkButtonUrl.startsWith("http") ? <a className="action-button" href={branding.cdkButtonUrl} target="_blank" rel="noopener noreferrer"><Icon name="payments" />前往店铺购买 CDK</a> : <Link className="action-button" href={branding.cdkButtonUrl}><Icon name="payments" />{branding.cdkButtonUrl === "/portal/redeem" ? "使用 CDK 兑换" : "前往店铺购买 CDK"}</Link>}
            {branding.cdkButtonUrl !== "/portal/redeem" ? <Link className="ghost-button" href="/portal/redeem"><Icon name="redeem" />使用 CDK 兑换</Link> : null}
          </div>
        </div> : null}
      </Drawer>
    </ConsoleShell>
