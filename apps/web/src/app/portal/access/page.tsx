...
export default function PortalAccessPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const nextAccess = await apiRequest<PortalAccessResponse>(
        "/api/portal/access",
        {
          token,
        },
  ...
  return (
    <ConsoleShell
      ...
      toolbarMeta={
        access ? (
          <span className="badge success">{access.nodes.length} 个节点</span>
        ) : null
      }
      toolbarActions={
        <button
      ...
      {!access && !emptyState && !error ? (
        <section className="workspace-grid">
          ...
          <div className="split">
            <div
              className="skeleton"
              style={{ height: 170, borderRadius: "var(--radius-md)" }}
            />
            <div
              className="skeleton"
              style={{ height: 170, borderRadius: "var(--radius-md)" }}
      ...
      {access ? (
        <section className="portal-access-layout">
          <div className="portal-access-subscription">
            <Panel
              title="一键订阅"
              copy="导入后自动显示套餐内全部节点，刷新订阅即可同步后台变更。"
            >
              <div className="subscription-import-card">
                <div className="subscription-import-head">
                  <div className="subscription-import-copy">
                    ...
                    <h3>复制订阅链接并导入客户端</h3>
                    <p className="muted">
                      v2rayN：订阅分组 → 添加订阅；Hiddify：从剪贴板添加配置。
                    </p>
                  </div>
                  <div
                    className="subscription-node-list"
                    aria-label="套餐可用节点"
                  >
                    {access.nodes.map((node) => (
                      <span className="badge" key={node.id}>
                        {node.label} ·{" "}
                        {node.protocol === "vless_reality" ? "VLESS" : "HY2"}
                      </span>
                    ))}
                  </div>
                </div>
                <label className="field">
                  ...
                  <div className="subscription-link-row">
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() => void copyText(subscriptionUrl)}
                    >
                      复制订阅链接
                    </button>
                  </div>
              ...
              <div className="form-grid">
                ...
                <div className="toolbar-actions">
                  <button
                    className="action-button"
                    type="button"
                    onClick={() => void copyText(access.uri)}
                  >
                    复制 URI
                  </button>
                  <button
                    ...
                    type="button"
                    onClick={() => void copyText(access.token)}
                  >
                    复制令牌
                  </button>
                </div>
          ...
          <div className="portal-access-side">
            ...
            <Panel
              title="推荐配置片段"
              copy={
                access.protocol === "vless_reality"
                  ? "当前节点的 Xray VLESS + REALITY 出站配置。"
                  : "带宽建议和本地代理端口放在 YAML 里而不是 URI 里。"
              }
            >
              ...
              <button
                className="ghost-button"
                type="button"
                onClick={() => void copyText(access.configSnippet)}
              >
                复制配置片段
              </button>
            </Panel>
        ...
        <Panel
          title="还没有可用接入信息"
          copy="当前账号还没有生效中的套餐或兑换尚未完成。先去兑换中心使用一张 CDK，系统会自动生成专属接入信息。"
        >
          <div className="toolbar-actions">
