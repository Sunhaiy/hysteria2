...
describe('plan redemption policy', () => {
  ...
  it('extends an unexpired subscription when renewing the same plan', () => {
    const result = resolvePlanRedemptionWindow({
      ...
      currentStartsAt: new Date('2026-01-31T08:00:00.000Z'),
    ...
    expect(result.endsAt.toISOString()).toBe('2026-07-31T08:00:00.000Z');
  });
  ...
  it('uses a fresh anchor when the previous subscription already expired', () => {
    const result = resolvePlanRedemptionWindow({
      mode: 'RENEW',
      currentPlanId: 'plan_pro',
      targetPlanId: 'plan_pro',
      currentStartsAt: new Date('2026-01-31T08:00:00.000Z'),
      currentEndsAt: new Date('2026-02-28T08:00:00.000Z'),
      redeemedAt: new Date('2026-03-10T08:00:00.000Z'),
      intervalMonths: 1,
      durationDays: 30,
    });

    expect(result.endsAt.toISOString()).toBe('2026-04-10T08:00:00.000Z');
  });
});
