...
const { plans, trafficPacks } = require('./catalog-preview-pricing');
...
if (process.env.CATALOG_PREVIEW_CONFIRM !== 'local-only') {
  throw new Error(
    'Set CATALOG_PREVIEW_CONFIRM=local-only to stage preview data',
  );
}
...
async function stagePack(tx, pack, index, nodeIds) {
  ...
  await tx.catalogOffer.upsert({
    where: { id: `catalog_offer_preview_pack_${pack.slug}_one_time` },
    create: {
      id: `catalog_offer_preview_pack_${pack.slug}_one_time`,
      productId: id.product,
      slug: `preview-pack-${pack.slug}-one-time-2026`,
      name: '一次性',
...
async function main() {
  ...
  console.log(
    `Staged ${plans.length} plans and ${trafficPacks.length} traffic packs locally.`,
  );
}
